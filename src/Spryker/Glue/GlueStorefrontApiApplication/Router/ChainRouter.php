...
class ChainRouter extends SymfonyChainRouter implements ChainRouterInterface
{
    ...
    public function routeResource(GlueRequestTransfer $glueRequestTransfer): GlueRequestTransfer
    {
        $this->setContext(
            (new RequestContext())
                ->setPathInfo($glueRequestTransfer->getPath())
                ->setMethod($glueRequestTransfer->getMethod())
                ->setHost($glueRequestTransfer->getHost())
                ->setQueryString($glueRequestTransfer->getParametersString()),
        );

        try {
            $matchResult = $this->match($glueRequestTransfer->getPath());
    ...
    /**
     * @param array<int, string> $matchResult
     *
